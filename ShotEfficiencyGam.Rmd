...
shot_selection_weighted_efficiency_summary %>% select(team, season, true_shot_selection_efficiency, Pyth_W_pct, win_perc) %>% arrange(desc(true_shot_selection_efficiency)) %>% head(10) %>% write_csv("data/best_teams.csv")
shot_selection_weighted_efficiency_summary %>% select(team, season, true_shot_selection_efficiency, Pyth_W_pct, win_perc) %>% arrange(true_shot_selection_efficiency) %>% head(10) %>% write_csv("data/worst_teams.csv")
```
...
shot_selection_weighted_efficiency_summary %>% ungroup() %>% select(Pyth_W_pct, W_pct,NRtg, true_shot_selection_efficiency,exogenous_shooting_ability, `off_TOV%`, `off_ORB%`, DRtg, payroll_normalized, season) -> selected_data
...
```{r}
shot_selection_weighted_efficiency_summary %>% inner_join(all_team_gam_data %>% rename(gam_shot_selection_efficiency = shot_selection_efficiency_coefficient), by = join_by(team, season)) -> shot_selection_weighted_efficiency_summary
```
...
lm(formula = Pyth_W_pct ~ DRtg + gam_shot_selection_efficiency + exogenous_shooting_ability + payroll_normalized + I(payroll_normalized^2) + team + season, data = shot_selection_weighted_efficiency_summary) -> true_shot_selection_model_fe14_payroll#peaks when payroll = 0.81229
summary(true_shot_selection_model_fe14_payroll)
true_shot_selection_model_fe14_payroll_stepVIF <- stepVIF(true_shot_selection_model_fe14_payroll, threshold = 10, verbose = TRUE)

tab_model(true_shot_selection_model_fe4,true_shot_selection_model_fe8, true_shot_selection_model_fe14_payroll, file="tables/gam_true_shot_selection_models.doc", digits = 3,show.se = T,show.ci=F)
```

```{r}
shot_selection_weighted_efficiency_summary %>% select(Pyth_W_pct, win_perc, team, season, gam_shot_selection_efficiency, exogenous_shooting_ability) %>% arrange(desc(exogenous_shooting_ability)) %>% head(10) %>% bind_rows(shot_selection_weighted_efficiency_summary %>% select(Pyth_W_pct, win_perc, team, season, gam_shot_selection_efficiency, exogenous_shooting_ability) %>% arrange(exogenous_shooting_ability) %>% head(10))
```
